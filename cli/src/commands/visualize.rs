...
/// Generate an interactive HTML visualization of the docs graph.
///
/// # Arguments
/// * `output` - Optional output directory (defaults to .graxus/viz/)
pub fn run_docs(ctx: &CliContext, output: Option<&str>) -> Result<()> {
...
/// Generate an interactive HTML visualization of the codemap.
///
/// # Arguments
/// * `output` - Optional output directory
pub fn run_codemap(ctx: &CliContext, output: Option<&str>) -> Result<()> {
    let root = ctx.resolve_root()?;
...
/// # Arguments
/// * `output` - Optional output directory
pub fn run_callgraph(ctx: &CliContext, output: Option<&str>) -> Result<()> {
    let root = ctx.resolve_root()?;
...
/// Generate an interactive HTML visualization of impact analysis for a target.
///
/// # Arguments
/// * `target` - File or symbol to analyze
/// * `output` - Optional output directory
pub fn run_impact(ctx: &CliContext, target: &str, output: Option<&str>) -> Result<()> {
    let root = ctx.resolve_root()?;
    ...
    println!(
        "\n{}",
        format!("Impact visualization for '{}' generated.", target)
            .green()
            .bold()
    );
    Ok(())
...
/// Generate an interactive HTML visualization of the docs-code bridge.
///
/// # Arguments
/// * `output` - Optional output directory
pub fn run_bridge(ctx: &CliContext, output: Option<&str>) -> Result<()> {
    let root = ctx.resolve_root()?;
    ...
    let bridge =
        graxus_agent_api::BridgeBuilder::build(&doc_graph, &graxus_codemap::CodeGraph::default())?;
...
/// # Arguments
/// * `output` - Optional output directory
pub fn run_deps(ctx: &CliContext, output: Option<&str>) -> Result<()> {
    let root = ctx.resolve_root()?;
...
/// Generate all visualizations (docs, codemap, callgraph, deps, bridge).
///
/// # Arguments
/// * `output` - Optional output directory
pub fn run_all(ctx: &CliContext, output: Option<&str>) -> Result<()> {
    let root = ctx.resolve_root()?;
    ...
    write_html(
        &out,
        "codemap.html",
        &graxus_viz::template::render_html(&d3),
    )?;
