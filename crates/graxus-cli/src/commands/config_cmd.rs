...
pub fn run_set_key(provider: &str, key: &str) -> Result<()> {
    ...
    let mut config = GraxusConfig::load(&root)?;
...
pub fn run_show() -> Result<()> {
    ...
    println!("    Max chars/file:  {}", config.defaults.max_chars_per_file);
    println!();
    ...
    println!("    API key:   {}", if config.embeddings.api_key_env.is_empty() { "(not set)".to_string() } else { config.embeddings.api_key_env.clone() });
    ...
    println!("    API key:   {}", if config.llm.api_key_env.is_empty() { "(not set)".to_string() } else { config.llm.api_key_env.clone() });
    ...
    if !config.embeddings.api_key_env.is_empty() {
        ...
        println!();
        println!("  Embeddings key: {}", if set { "✓ set".green().to_string() } else { "✗ not set".red().to_string() });
    }
    if !config.llm.api_key_env.is_empty() {
        let set = std::env::var(&config.llm.api_key_env).is_ok();
        println!("  LLM key:        {}", if set { "✓ set".green().to_string() } else { "✗ not set".red().to_string() });
    }

    Ok(())
}

pub fn run_update(key: &str, value: &str) -> Result<()> {
    let cwd = env::current_dir()?;
    let root = workspace::find_root(Path::new(&cwd))
        .context("Not a graxus project. Run `graxus init` first.")?;
    ...
    match key {
        ...
        "min_confidence" | "defaults.min_confidence" => config.defaults.min_confidence = parse_f64(value)?,
        "context_budget" | "defaults.context_budget" => config.defaults.context_budget = parse_usize(value)?,
        ...
        _ => anyhow::bail!("Unknown config key: '{}'. Use dot notation like 'context.depth'", key),
    }
...
fn parse_usize(value: &str) -> Result<usize> {
    value.parse().map_err(|_| anyhow::anyhow!("Invalid number: '{}'", value))
}
