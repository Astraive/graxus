...
impl FrameworkResolver for EchoResolver {
    ...
    if node.kind() == "import_spec" {
        if let Some(path_node) = node.child_by_field_name("path") {
            if let Some(path) = go_string_literal(path_node, source) {
                if is_echo_import(&path) {
                    if let Some(package_name) = import_package_name(node, path_node, source, "echo")
                    {
...
fn contains_framework_type(node: Node<'_>, source: &str, package_names: &HashSet<String>) -> bool {
    ...
    package_names.iter().any(|package_name| {
        text.contains(&format!("{package_name}.Echo"))
            || text.contains(&format!("{package_name}.Group"))
    })
}
...
fn is_echo_import(path: &str) -> bool {
    matches!(
        path,
...
mod tests {
    ...
    #[test]
    fn extracts_echo_routes_without_claiming_fiber_routes() {
        let source = r#"
package routes
...
"#;
        ...
        let routes = EchoResolver.extract_routes("routes.go", source);
