    ...
    if node.kind() == "import_spec" {
        if let Some(path_node) = node.child_by_field_name("path") {
            if let Some(path) = go_string_literal(path_node, source) {
                if is_gin_import(&path) {
                    if let Some(package_name) = import_package_name(node, path_node, source, "gin")
                    {
...
fn is_framework_constructor(node: Node<'_>, source: &str, package_names: &HashSet<String>) -> bool {
    ...
    package.kind() == "identifier"
        ...
        && node_text(method, source).is_some_and(|name| name == "New" || name == "Default")
}
...
fn contains_framework_type(node: Node<'_>, source: &str, package_names: &HashSet<String>) -> bool {
    ...
    package_names.iter().any(|package_name| {
        text.contains(&format!("{package_name}.Engine"))
            || text.contains(&format!("{package_name}.RouterGroup"))
    })
}
...
fn is_gin_import(path: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn extracts_gin_routes_without_claiming_echo_routes() {
        let source = r#"
package routes
...
"#;
        ...
        let routes = GinResolver.extract_routes("routes.go", source);
        ...
        let dispatched_routes = super::super::extract_routes("routes.go", source, "go");
        assert_eq!(dispatched_routes.len(), 3);
        assert_eq!(
            dispatched_routes
                .iter()
                .filter(|route| route.framework == "gin")
                .count(),
            2
        );
        assert_eq!(
            dispatched_routes
                .iter()
                .filter(|route| route.framework == "echo")
                .count(),
            1
        );
