...
pub struct RocketResolver;

impl FrameworkResolver for RocketResolver {
    ...
    fn extract_routes(&self, file: &str, source: &str) -> Vec<RouteFact> {
        ...
        if !has_rocket_evidence(tree.root_node(), source) {
            return Vec::new();
        }
...
fn has_rocket_evidence(node: Node<'_>, source: &str) -> bool {
    match node.kind() {
        "use_declaration" | "extern_crate_declaration" => {
            if imports_crate(node_text(node, source).unwrap_or_default(), "rocket") {
                return true;
            }
        }
        "attribute_item" => {
            if node_text(node, source)
                .is_some_and(|attribute| attribute.trim_start().starts_with("#[rocket::"))
            {
                return true;
            }
        }
        "scoped_identifier" => {
            if node_text(node, source).is_some_and(|path| path.starts_with("rocket::")) {
                return true;
            }
        }
        _ => {}
    ...
    (0..node.named_child_count()).any(|index| {
        node.named_child(index)
            .is_some_and(|child| has_rocket_evidence(child, source))
    })
}
...
mod tests {
    ...
    #[test]
    fn extracts_rocket_attribute_routes() {
        let source = r#"use rocket::{delete, get, patch, post, put};
...
fn delete_user() {}"#;
    ...
    #[test]
    fn requires_rocket_source_evidence() {
        assert!(resolver()
    ...
    #[test]
    fn ignores_incomplete_rocket_syntax() {
        assert!(resolver()
            .extract_routes("src/routes.rs", "use rocket::get;\n#[get(\"/users\")]",)
            .is_empty());
    }
}
