...
pub struct DrogonResolver;

impl FrameworkResolver for DrogonResolver {
    ...
    fn extract_routes(&self, file: &str, source: &str) -> Vec<RouteFact> {
        ...
        for start in invocation_offsets(source, "ADD_METHOD_TO") {
            let open = skip_whitespace(source, start + "ADD_METHOD_TO".len());
            ...
            let handler = arguments
                .first()
                .map_or_else(String::new, |argument| argument.trim().to_string());
            let mut methods = Vec::new();
            for argument in arguments.iter().skip(2) {
                extend_unique_methods(&mut methods, collect_http_methods(argument));
            }
...
mod tests {
    ...
    #[test]
    fn extracts_drogon_method_tokens_and_deduplicates_them() {
        let source = r#"
ADD_METHOD_TO(UsersController::list, "/users", Get, Options, Get);
ADD_METHOD_TO(UsersController::create, "/users/{1}", drogon::Post);
ADD_METHOD_TO(UsersController::ignored, dynamic_path, Get);
ADD_METHOD_TO(UsersController::incomplete, "/unfinished", Get
"#;

        let routes = resolver().extract_routes("controllers/users.cc", source);

        assert_eq!(routes.len(), 3);
        assert!(routes.iter().any(|route| {
            route.method == "GET"
                && route.path == "/users"
                && route.handler == "UsersController::list"
                && route.framework == "drogon"
                && route.file == "controllers/users.cc"
                && route.line == 2
        }));
        assert!(routes.iter().any(|route| {
        ...
        assert!(routes.iter().any(|route| {
            route.method == "POST"
                && route.path == "/users/{1}"
                && route.handler == "UsersController::create"
                && route.line == 3
        }));
    }
}
