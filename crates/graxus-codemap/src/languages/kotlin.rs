//! Kotlin language indexer using tree-sitter.
...
/// Bridge tree-sitter-kotlin-ng to the workspace tree-sitter.
...
pub struct KotlinIndexer {
    ...
    sym_object_query: Query,
    call_ident_query: Query,
}

impl KotlinIndexer {
    pub fn new() -> Self {
        let lang = kotlin_language();
        Self {
            import_query: Query::new(&lang, r#"(import (identifier) @module) @import"#)
                .expect("valid kotlin import query"),
            sym_func_query: Query::new(
                &lang,
                r#"(function_declaration name: (identifier) @name) @def"#,
            )
            .expect("valid kotlin func query"),
            sym_class_query: Query::new(
                &lang,
                r#"(class_declaration name: (identifier) @name) @def"#,
            )
            .expect("valid kotlin class query"),
            sym_object_query: Query::new(
                &lang,
                r#"(object_declaration name: (identifier) @name) @def"#,
            )
            .expect("valid kotlin object query"),
            call_ident_query: Query::new(&lang, r#"(call_expression (identifier) @callee) @call"#)
                .expect("valid kotlin call ident query"),
        }
    }
}

impl Default for KotlinIndexer {
    fn default() -> Self {
        Self::new()
    }
}
